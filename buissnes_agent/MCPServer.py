...
@mcp.tool(description=os.getenv("ISO20022_BUSINESS_TOOL_DESC", """
...
@mcp.tool(description=os.getenv("ISO20022_TECHNICAL_TOOL_DESC", """
...
@mcp.tool(description=os.getenv("ISO20022_MESSAGE_SCHEMAS_TOOL_DESC", """
...
@mcp.tool(description=os.getenv("SYSTEM_DOCUMENTATION_TOOL_DESC", """
    Wyszukuje informacje w dokumentacji systemów zawierającej konkretne specyfikacje systemów.
    Użyj tego narzędzia do pytań o systemy:
    - SWIFT CBPR+.
    - TARGET2.
    - TIPS.
    - SCT Inst.
    """))
async def query_system_documentation(query: str) -> str:
    """Internal function description (ignored if description is provided above)."""
    collection_name = os.getenv('SYSTEM_DOCUMENTATION_COLLECTION_NAME')
    # WAŻNE: asyncio.to_thread uruchamia funkcję synchroniczną (run_iso_rag) w osobnym wątku.
    # Zapobiega to blokowaniu pętli zdarzeń (Event Loop) serwera, gdy czekamy na bazę danych.
    return await asyncio.to_thread(run_generic_rag, query, collection_name)
